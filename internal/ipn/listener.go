...
// notification asked (a payment that granted nothing, say).
func (l *Listener) apply(ctx context.Context, tx pgx.Tx, id int64, n *Notification) (string, error) {
	now := l.now()
...
WHERE provider = $1 AND external_id = $2 RETURNING guild_id::text`, provider, sub).Scan(&guildID)
		...
		return "", recompute(ctx, tx, *guildID, now)
...
func recompute(ctx context.Context, tx pgx.Tx, guildID string, now time.Time) error {
...
func (l *Listener) Reconcile(ctx context.Context) error {
	...
	for _, g := range guilds {
		err := l.DB.BeginFunc(ctx, func(tx pgx.Tx) error { return recompute(ctx, tx, g, now) })
		if err != nil {
