...
func TestBearerRoutes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, path := range []string{"/guild/settings?guildID=" + guild, "/guild/premium?guildID=" + guild, "/guild/bot?guildID=" + guild, "/guild/stats?guildID=" + guild, "/game/state?guildID=" + guild + "&connectCode=ABCDEFGH", "/game/roomcode?guildID=" + guild + "&connectCode=ABCDEFGH"} {
