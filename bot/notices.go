...
func (bot *Bot) handleEvent(e *notice.Event) {
	...
	case e.GamesAvailable != nil:
		bot.adoptGames(e.GamesAvailable.Games)
...
func (bot *Bot) stopGame(gsr GameStateRequest, reason server.EndReason, message string) error {
	...
	if !ok {
		// A replica can receive /end before it discovers the game's subscription.
		// Its cleanup must still wait for capture work on other replicas to finish.
		lease := bot.newConsumerLease(gsr.ConnectCode)
		if lease.client == nil {
			return errNoRedis
		}
		lease.acquireBlocking("ending game without a local subscriber")
		defer lease.release(true)
		// completeGame rechecks existence after acquiring the lease: another
		// consumer may have ended the game while we were waiting.
...
func (bot *Bot) completeGame(gsr GameStateRequest, reason server.EndReason, message string) error {
	current, err := bot.store.ReadDiscordGameState(gsr)
	if err != nil {
		return err
	}
	if current == nil {
